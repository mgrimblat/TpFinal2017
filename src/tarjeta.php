  ...
  public function pagar_viaje(transporte $transporte, $fecha){
    ...
    if($transporte->get_tipo() == "Colectivo"){
      ...
      return;
        ...
        if($this->saldo >= 12.45){
          $this->saldo -= 12.45;
          $this->viajes[] = new viaje(0, 12.45, $transporte->tipo, strtotime($fecha));
        }
        else {
        ...
        $this->viajes[] = new viaje(0, 0.0, $transporte->tipo, strtotime($fecha));
      }
    }
    ...
    protected function pagar_colectivo(transporte $transporte, $fecha){
      ...
      if($this->saldo >= $monto){
        $this->saldo -=$monto;
        $this->viajes[] = new viaje(0, $monto, "Colectivo", strtotime($fecha));
      }
    }
